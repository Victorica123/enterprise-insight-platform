...
RocketMQ 不加快单个视频，也不会凭空增加 worker 吞吐。它把高峰期线程池拒绝造成的 HTTP 失败，转换成 broker 中可恢复、可观测的排队。
...
更长压测中，两种模式处理吞吐都约为 1.3/s；MQ 开启后积压任务的真实端到端等待最高约 1354 秒。这证明“请求接住了”不等于“处理变快了”。
...
已实现并验证：70 个自动化测试、真实 Redis/MySQL/RocketMQ 链路、MinIO 直传与播放、MQ A/B、重试/reaper、Compose/Caddy/preflight。
...
> 我做的是一个多用户视频内容理解平台，重点不是 CRUD，而是大文件上传和异步 AI 工作流。上传侧有普通上传、Redis 分片断点续传和 MinIO 预签名直传；处理侧用本地异步或 RocketMQ 解耦，通过状态机 claim、内容级 single-flight、失败重试和 stale-task reaper 保证可恢复。真实 MQ A/B 证明 MQ 能把过载时约 90% 的 HTTP 失败转成内部排队，但不会增加单任务速度和 worker 吞吐。项目有 70 个自动化测试、真实中间件 smoke、Prometheus/Grafana 和小规模 Compose/Caddy 部署方案，同时我明确保留了数据库迁移、上传会话审计、限流和备份恢复这些生产化缺口。
