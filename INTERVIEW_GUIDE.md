...
已实现并验证：85 个自动化测试、真实 Redis/MySQL/RocketMQ 链路、MinIO 直传与播放、页面异步 A/B、重试/reaper、任务配额、媒体生命周期清理、Compose/Caddy/preflight。
...
> 我做的是一个多用户视频内容理解平台，重点不是 CRUD，而是大文件上传和异步 AI 工作流。上传侧有普通上传、Redis 分片断点续传和 MinIO 预签名直传；处理侧用本地异步或 RocketMQ 解耦，通过状态机 claim、内容级 single-flight、失败重试、用户任务配额和 stale-task reaper 保证可恢复。页面同参数 A/B 中，本地线程池只接收 54/80，RocketMQ 接收 80/80；两边都是 4 个 worker，单任务成本不变，差异来自削峰和补偿等待。任务删除采用持久化清理任务处理存储失败，过期分片会自动回收。项目有 85 个自动化测试和真实中间件 smoke，不依赖公网服务器也能完整复现。
