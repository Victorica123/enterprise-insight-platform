...
成员角色变化与 IdP 禁用在下一次重签 JWT 后生效；已签发的短期 Workspace 令牌在自然过期前仍可能有效。若试点要求近实时撤权，应增加令牌版本/集中撤销检查。Agent 八项编号迁移和 Media Flyway V1/V2 已在隔离 MySQL 8.4 新旧库验证，并完成两个业务库备份还原；现有业务库未直接升级。本轮使用 HS256 与 mock/local AI，公开发布仍需正式身份集成、目标库副本升级/回退、密钥轮换、对象存储与异机灾备、供应商数据处理协议验证。
