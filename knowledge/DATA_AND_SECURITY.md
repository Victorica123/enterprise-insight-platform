...
- 阶段记录使用任务同一授权范围，viewer 可以读取但不能写入媒体/文档/分析；前端同步禁用这些写入口。记录只暴露阶段、runId、状态、时间、耗时与固定错误码，沿用 audit-days 保留期，不含原文、存储路径、令牌或租约凭证。
...
Web Query 缓存按 tenant/user/role/workspaceType 隔离，身份、Workspace 或角色变化时取消请求并清空旧缓存及会话状态，跨标签页更新同样生效。旧请求迟到不能进入新范围；这不替代后端 JWT、成员关系与四眼检查。

会话领取、读取与续轮沿用个人 tenant + owner / 团队 tenant 范围，不能用 conversation ID 扩大权限。持久化保存原问题、改写问题、最终回答、来源快照与逐轮 actor；读取历史时刷新治理来源状态。历史只提供主题提示，不能把已删除或撤回证据变成当前事实。

摘要调用受同一 tenant 模型出境策略与调用预算约束；新增的客户/项目标识触发规则降级。相邻块扩展再次限制文档、章节、scope、主题与字符预算；主题过滤是相关性规则，不是权限过滤的替代。

租约和 revision 防止同会话竞争提交，失效执行者不能清除新执行者的预算预留。取消时只退还确定未用的额度，不保存未完成回答；在服务端观察取消前完成的轮次仍可能保留。SSE 仅发送脱敏错误，待核验片段不作为最终来源证明。

会话历史沿用 `RETENTION_AUDIT_DAYS`（默认 365 天），由既有保留任务有界清理并跳过活跃租约；没有新增独立保留期限。会话历史接口最多返回 50 轮，记忆窗口只读取四轮用户问题，不把完整历史发给模型。
...
成员角色变化与 IdP 禁用在下一次重签 JWT 后生效；已签发的短期 Workspace 令牌在自然过期前仍可能有效。若试点要求近实时撤权，应增加令牌版本/集中撤销检查。Agent 八项编号迁移和 Media Flyway V1/V2 已在隔离 MySQL 8.4 新旧库验证，并完成两个测试业务库备份还原；2026-09-13 后续也已完成原本机 H2/SQLite 的副本演练与正式升级，见 [OPERATIONS](OPERATIONS.md)。当前本机使用 HS256 与 mock/local AI，公开发布仍需正式身份集成、目标生产库副本升级/回退、密钥轮换、对象存储与异机灾备、供应商数据处理协议验证。
