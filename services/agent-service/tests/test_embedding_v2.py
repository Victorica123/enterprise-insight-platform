...
class HybridTieBreakTests(unittest.TestCase):
    ...
    64 维哈希向量的噪声分反超了 "退款" 这类精确命中。
    """

    def setUp(self) -> None:
        self.temp_dir = tempfile.TemporaryDirectory()
        self.db_patcher = patch("app.database.DB_PATH", Path(self.temp_dir.name) / "tie.sqlite3")
        self.db_patcher.start()
        clear_chunk_cache()
        ingest_document("hiring.md", "招聘产品负责人要求 P0，验收标准为三天内完成简历筛选。")
        ingest_document("refund-sla.md", "退款产品负责人要求 P0 优先，验收标准为退款申请 2 秒内受理。")
