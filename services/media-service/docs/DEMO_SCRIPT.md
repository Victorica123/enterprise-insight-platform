# Media Service 5-10 分钟面试深挖脚本

目标：在统一产品主演示之后，按需深挖 Media Service，说明 Redis、RocketMQ、MinIO 和可靠性设计如何解决真实问题。它不是第二个项目的独立演示脚本。
...
> 这是 Enterprise Insight Platform 的媒体证据子系统。用户上传视频后，系统异步完成音频提取、语音转写和 AI 摘要，并把带时间戳的转写通过 outbox 交给 Agent Service。这里重点解决三个工程问题：大文件上传不稳定、耗时任务拖垮请求链路，以及多人使用时文件和任务必须严格隔离。
