...
CREATE TABLE workspace (
    ...
    workspace_type ENUM('PERSONAL','TEAM'), created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL
);
CREATE TABLE workspace_member (
    member_id VARCHAR(255) NOT NULL PRIMARY KEY, tenant_id VARCHAR(64) NOT NULL, user_id VARCHAR(64) NOT NULL,
    role ENUM('ADMIN','MEMBER','OWNER','VIEWER') NOT NULL, joined_at TIMESTAMP(6) WITH TIME ZONE NOT NULL,
...
CREATE TABLE workspace_invitation (
    ...
    created_by VARCHAR(64) NOT NULL, created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL, expires_at TIMESTAMP(6) WITH TIME ZONE NOT NULL,
    accepted_by VARCHAR(64), accepted_at TIMESTAMP(6) WITH TIME ZONE,
...
CREATE TABLE video_task (
    ...
    processing_lease_id VARCHAR(255), processing_lease_expires_at TIMESTAMP(6) WITH TIME ZONE,
...
CREATE TABLE media_cleanup_job (
    ...
    last_error TEXT, next_attempt_at TIMESTAMP(6) WITH TIME ZONE, created_at TIMESTAMP(6) WITH TIME ZONE, updated_at TIMESTAMP(6) WITH TIME ZONE
);
CREATE TABLE workflow_dispatch_outbox (
    ...
    next_attempt_at TIMESTAMP(6) WITH TIME ZONE NOT NULL, last_error TEXT, created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL, dispatched_at TIMESTAMP(6) WITH TIME ZONE,
    claim_id VARCHAR(64), claim_expires_at TIMESTAMP(6) WITH TIME ZONE
);
...
CREATE TABLE integration_event_outbox (
    ...
    payload TEXT NOT NULL, status ENUM('CLAIMED','DEAD','PENDING','SENT') NOT NULL, attempts INTEGER NOT NULL, next_attempt_at TIMESTAMP(6) WITH TIME ZONE NOT NULL,
    last_error TEXT, created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL, published_at TIMESTAMP(6) WITH TIME ZONE, claim_id VARCHAR(64), claim_expires_at TIMESTAMP(6) WITH TIME ZONE
);
