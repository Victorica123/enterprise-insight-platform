...
2026-09-14 指标解释补充：下方历史记录的 `recall3` 按问题统计“Top-3 来源命中任一目标文档”，更准确称 Hit@3；`fact_coverage` 是任一预期事实子串命中，不是完整事实覆盖或逐句真实性。本次只校正文档口径，未改评测代码或历史结果；详见 [评测答辩](INTERVIEW_GUIDE.md#evaluation)。

代码门禁：最终 Agent **271/271**、会话专项 **25/25**、Media **150/150**（JDK 17.0.18）、Web **26/26**；Python lint 通过，Web lint 0 error / 10 warnings，构建通过。V6 45 题，hybrid 决策 **44/45**、Top-3 来源命中与事实子串命中均 **39/40**；PRD **12/12**、生命周期、V5 通过；Conversation 最终 **14/14**。这些是小型闭集回归，不是开放域准确率。
