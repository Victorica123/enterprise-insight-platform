...
- 选型报告：2026-09-15 完成 LangGraph 接入设计；当前业务实现仍沿用原编排。
...
以 `5c136b4` 为代码基线，用户同意 LangGraph 选型方向并要求应用与比较报告。[ADR-0020](../knowledge/decisions/0020-langgraph-application-report.md) 同时承担报告和决策记录，涵盖现有代码映射、条件图、独立示例、框架比较及验收；面试指南收缩为口述与报告入口。报告建议先迁移六阶段调度、复用业务检查点，不同时引入第二套审批状态存储。当前尚未迁移应用、修改依赖或部署；验证见 [QUALITY](../knowledge/QUALITY.md)。下一步实施从报告 B 阶段开始，并保留证据冻结、阶段 1–4 和令牌 CAS 不变量。
